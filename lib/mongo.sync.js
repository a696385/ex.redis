 ...
 * @param {Function} transform Transform Object
 * @param {Function} [callback]
 */
exports.sync = function(mongo, redis, transform, callback){
    ...
    var copyFromMongoToRedis = function(version, callback){
        async.waterfall([
            ...
            function(callback){ mongoCollection.find({}, callback); },
            ...
            function(results, callback){
                async.each(results, function(doc, next){
                    ...
                    var value = transform(doc._id.toHexString(), doc, 0);
                    if (value){
                        syncObjects++;
                        exRedis.write(redis.connection, redis.collection, value.key, value.data, next);
                    } else {
                        next();
                    }
                }, callback);
        ...
        async.waterfall([
            ...
            function(results, callback){
                async.each(results, function(doc,next){
                    ...
                    var key = doc.__key;
                    delete doc.__key;
                    var value = transform(key, doc, 1);
                    if (value){
                        mongoCollection.insert(value, next);
                    } else {
                        next();
                    }
                }, callback);
